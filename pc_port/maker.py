...
class Editor:
	...
	def loadImageData(self, img_url):
		...
		self.image = self.image.resize((256, 256), Image.LANCZOS)
	...
	def loadBinaryData(self):
		...
		if len(data) > size+4 and data[size:size+0x4] == B"ASET":
				...
				if hasPIL and len(asset.icon) > 0:
...
if hasGUI:
	...
	if "icon" in config.keys():
		if hasPIL:
			...
			print(f"-> Going to update the icon to {config['icon']}!")
		else:
